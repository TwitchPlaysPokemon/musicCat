...
A Menu System is organized into a hierarchy.  There is a top level Menu object,
which has a set of choices.  Any / All / None of this Menu's Choice objects can
contain sub-menus.  The Menu System hierarchy is extended downward by creating
...
class Menu:
    ...
    choices per menu, but aesthetically and organizationally more than 10 will
    likely cause problems and should be divided into sub-menus.

    A Menu object is capable of displaying itself and of retrieving a choice
    ...
    beneath it in the hierarchy as needed.
    """
    ...
    def waitForInput(self):
        """Main event loop, progresses indefinitely
        ...
        while loop != False: # Specifically check for False; handlers may not return anything (None), or another false-ish value
            ...
            if c in self.choices:
                ...
                if self[c].handler and not self[c].subMenu:
                    try:
                        loop = self[c].handler(self[c].value) if self[c].value else self[c].handler()
                ...
                if self[c].subMenu:
                    ...
                    if self[c].handler:
                        try:
                            if hasattr(self[c].subMenu, 'data'):
                                data = self[c].subMenu.data
...
class DataMenu(Menu):
    ...
    """
    def __init__(self, title='', prompt='', valid=None, err="Invalid, try again.", abort=True):
        ...
        values into a constructor.  All parameters are optional.
                  ...
                  title -- Menu object's title
                 prompt -- Menu object's prompt
               validate -- Takes input, returns None if input was invalid, and the data if it was valid.
                           If omitted, no validation occurs and input is stored
                  abort -- If validation fails, return to menu with null data.
                    err -- Prompt for invalid input
        """
        self.data = None
        self.title = title
        self.prompt = prompt
    ...
    def waitForInput(self):
        ...
        Validation is performed on the data, to verify it's input correctly and to convert
        it to a workable value. If Abort allows it, re-prompt the user to enter data if its
        invalid. Otherwise, on invalid data, it is left as None.
...
class Choice:
    ...
    """
    def __init__(self, selector=1, description='', value=None, subMenu=None, handler=None):
        ...
        will be switched to the sub menu
        ...
        out what choice was selected by the end user.  This value can be converted by a Validation function.
